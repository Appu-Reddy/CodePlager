...
export default function StudentLoginPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      // Check for saved roll number in localStorage
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (data.message !== "Login successful") {
        ...
        // Save roll number if remember me is checked
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            <div className="flex items-center">
              <div className={`flex items-center ${darkMode ? 'text-green-500' : 'text-green-600'}`}>
            ...
            <div className="flex items-center space-x-4">
              ...
              <Link href="/teacher-login" className={`inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm transition-all duration-200 ease-in-out ${
                darkMode 
        ...
        <div className={`max-w-md w-full space-y-8 p-8 rounded-xl shadow-xl ${
          ...
          <div>
            ...
            <p className={`mt-2 text-center text-sm ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
              Access your courses, grades, and academic resources
            </p>
          </div>
          ...
          <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
            <div className="space-y-6">
              <div>
                <label htmlFor="roll-number" className={`block text-sm font-medium mb-1 ${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                  Roll Number
                </label>
          ...
          <div className={`pt-4 text-center text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            <p>Don't have an account? <a href="#" className={`font-medium ${darkMode ? 'text-green-400 hover:text-green-300' : 'text-green-600 hover:text-green-500'}`}>Contact administration</a></p>
          </div>
        </div>
      </div>
